...
static char *get_esp_chip_name(esp_chip_model_t model)
{
    switch (model) {
        case CHIP_ESP32:
            return "ESP32";
...
static char
printableTaskState(eTaskState state)
{
    switch (state) {
        case eReady:
            return 'r';
...
static int tasks_info(int argc, char **argv)
{
    ...
    if( pxTaskStatusArray != NULL )
    {
        ...
        printf("%-18s\tId\tState\tCPri\tBPri\t%-10s\tPct\tHWM\tSSize\tUsed\t\tStk Base\n", "Name", "Ctr");
        for( size_t x = 0; x < uxArraySize; x++ ) {

            int percentage = (ulTotalRunTime > 0) ? (pxTaskStatusArray[x].ulRunTimeCounter/ulTotalRunTime) : 0;

            uint32_t stackSize = uxTaskGetStackSize(pxTaskStatusArray[x].xHandle);
            ...
            printf("%-18s\t%d\t%c\t%d\t%d\t0x%08x\t%2d%%\t%04x\t%04x\t%04x (%2d%%)\t%08p\n",
                   ...
                   stackSize,
                   stackSize - pxTaskStatusArray[x].usStackHighWaterMark,
                   stackUsedPct,
